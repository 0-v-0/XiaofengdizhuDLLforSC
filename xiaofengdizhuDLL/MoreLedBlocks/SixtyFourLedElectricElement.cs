...
namespace Game
{
    public class SixtyFourLedElectricElement : MountedElectricElement
    {
        ...
        public override void OnAdded()
        {
            ...
            m_color = LedBlock.LedColors[SixtyFourLedBlock.GetColor(data)];
            for (int i = 0; i < 64; i++)
            {
                ...
                m_glowPoints[i].Position = v - 0.4375f * CellFace.FaceToVector3(mountingFace) + vector3 * ((i % 8 * -2f + 7f) * 0.0625f) + vector2 * ((i / 8 * -2f + 7f) * 0.0625f);
                m_glowPoints[i].Forward = vector;
                m_glowPoints[i].Up = vector2;
                m_glowPoints[i].Right = vector3;
                m_glowPoints[i].Color = Color.Transparent;
                m_glowPoints[i].Size = 0.07f;
                m_glowPoints[i].FarSize = 0.07f;
